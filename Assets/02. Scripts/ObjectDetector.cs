...
namespace LuckyDefense
{
    public class ObjectDetector : MonoBehaviour
    {
        ...
        [SerializeField] private TowerUIManager towerUIManager;
        ...
        {
            if (towerUIManager == null || selectedTowerGroup == null) return;
            
            Vector2Int slot = GetSlotFromPosition(selectedTowerGroup.transform.position);
            if (slot.x == -1) return;
            
            bool canMix = towerManager != null && towerManager.CanMixTower(slot.x, slot.y);
            
            if (canMix)
            {
                towerUIManager.OpenTowerMix(selectedTowerGroup.transform);
            }
            else
            {
                towerUIManager.CloseTowerMix();
            }
            
            towerUIManager.OpenTowerSell(selectedTowerGroup.transform);
        }
        ...
        {
            if (selectedTowerGroup == null) return;
            
            Vector2Int slot = GetSlotFromPosition(selectedTowerGroup.transform.position);
            if (slot.x == -1) return;
            
            if (towerManager != null && towerManager.MixPlayerTower(slot.x, slot.y))
            {
                Debug.Log("타워 합성 성공");
            }
        ...
        public void OnSellButtonClicked()
        {
            ...
            if (towerManager != null && towerManager.SellPlayerTower(slot.x, slot.y))
            {
                Debug.Log("타워 판매 성공");
                
                TowerGroup remainingTower = towerManager.GetPlayerTowerGroup(slot.x, slot.y);
                
                if (remainingTower == null)
                {
                    ClearSelection();
                }
                else
                {
                    selectedTowerGroup = remainingTower;
                    ShowTowerUI();
                }
            }
        }
